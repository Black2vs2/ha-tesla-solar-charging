...
void TeslaBLEVehicle::setup() {
  ...
  // Initialize polling mode select
...
void TeslaBLEVehicle::update() {
  ...
  if (current_polling_mode_ == PollingMode::OFF) {
    return;
  }

  uint32_t now = millis();

  // LAZY mode: connect-poll-disconnect cycle
  if (current_polling_mode_ == PollingMode::LAZY) {
    if (lazy_poll_pending_) {
      // Connected and polled — check grace period
      if (now - lazy_poll_started_at_ >= lazy_disconnect_after_ms_) {
        ESP_LOGI(TAG, "Lazy: grace period expired, disconnecting");
    ...
    // Check if time for a new poll
    if (now - last_vcsec_poll_ >= polling_interval_ms_) {
      ESP_LOGI(TAG, "Lazy: connecting for poll");
      this->parent()->set_enabled(true);
      lazy_poll_pending_ = true;
      lazy_poll_started_at_ = now;
      // Fall through only if already connected
  ...
  // ACTIVE/CLOSE or LAZY just connected: do actual polling
  if (!is_connected() || !vehicle_) return;
  ...
  if (now - last_infotainment_poll_ >= polling_interval_ms_) {
    ESP_LOGI(TAG, "Polling Infotainment (mode=%d)", (int)current_polling_mode_);
...
void TeslaBLEVehicle::dump_config() {
  ...
  ESP_LOGCONFIG(TAG, "  Polling Mode: %d (0=OFF, 1=LAZY, 2=ACTIVE, 3=CLOSE)", (int)current_polling_mode_);
...
void TeslaBLEVehicle::set_polling_mode_select(select::Select *sel) {
  polling_mode_select_ = sel;
}
...
void TeslaBLEVehicle::apply_polling_mode_(PollingMode mode) {
  if (mode == current_polling_mode_) return;

  ESP_LOGI(TAG, "Polling mode: %d -> %d", (int)current_polling_mode_, (int)mode);
  current_polling_mode_ = mode;
  polling_interval_ms_ = get_polling_interval_ms_(mode);
  ...
  if (mode == PollingMode::OFF) {
    ...
    ESP_LOGI(TAG, "Polling %s — interval %u ms",
...
void TeslaPollingModeSelect::control(const std::string &value) {
  this->publish_state(value);
  if (parent_) {
    auto mode = parent_->parse_polling_mode_(value);
    parent_->apply_polling_mode_(mode);
  }
}
...
void TeslaBLEVehicle::set_climate_temp(float temp) {
  ...
  if (state_manager_)
    state_manager_->track_command_issued();
  if (vehicle_)
    vehicle_->set_climate_temp(temp);
}
...
void TeslaBLEVehicle::set_climate_keeper(int mode) {
  ...
  if (state_manager_)
    state_manager_->track_command_issued();
  if (vehicle_)
    vehicle_->set_climate_keeper(mode);
}
...
void TeslaBLEVehicle::set_bioweapon_mode(bool enable) {
  ...
  if (state_manager_)
    state_manager_->track_command_issued();
  if (vehicle_)
    vehicle_->set_bioweapon_mode(enable);
}
...
void TeslaBLEVehicle::set_preconditioning_max(bool enable) {
  ...
  if (state_manager_)
    state_manager_->track_command_issued();
  if (vehicle_)
    vehicle_->set_preconditioning_max(enable);
}
...
void TeslaBLEVehicle::set_steering_wheel_heat(bool enable) {
  ...
  if (state_manager_)
    state_manager_->track_command_issued();
  if (vehicle_)
    vehicle_->set_steering_wheel_heat(enable);
}
...
void TeslaBLEVehicle::set_sentry_mode(bool enable) {
  ...
  if (state_manager_)
    state_manager_->track_command_issued();
  if (vehicle_)
    vehicle_->set_sentry_mode(enable);
}
