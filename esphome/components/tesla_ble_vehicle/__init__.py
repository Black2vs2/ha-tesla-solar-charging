...
# Custom select class
TeslaPollingModeSelect = tesla_ble_vehicle_ns.class_("TeslaPollingModeSelect", select.Select)
...
POLLING_MODE_SELECT = {
    "id": "polling_mode",
    "name": "Polling Mode",
    "class": TeslaPollingModeSelect,
    "setter": "set_polling_mode_select",
    "icon": "mdi:timer-cog",
    "options": ["off", "lazy", "active", "close"],
    "initial_option": "lazy",
}
...
async def create_select_entity(var, definition):
    """Create a select entity and register with TeslaBLEVehicle."""
    config = {
    ...
    cg.add(sel.set_initial_option(definition["initial_option"]))
    if definition.get("setter"):
        cg.add(getattr(var, definition["setter"])(sel))
    return sel
...
async def to_code(config):
    ...
    # Polling mode select
    await create_select_entity(var, POLLING_MODE_SELECT)
